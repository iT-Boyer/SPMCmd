...
var subtitleCommand = Command(
  usage: "subtitle", configuration: configuration, run: execute)
...
private func configuration(command: Command) {
    command.shortMessage = "download WebVTT files, change to text"
    command.longMessage = "Download each session's WebVTT file, change to text, write to disk."
...
fileprivate func path(for session: Session) -> String {
    return [path, String(session.year), "\(session.number)_\(session.title).txt"].joined(separator: "/")
}
...
fileprivate func normalize(_ vttText: String) -> String {
    ...
    text = removeTimes(from: text)
    text = addReturns(from: text)
    text = removeOverSpace(from: text)
    return text
}
...
fileprivate func addReturns(from webVttText: String) -> String {
    let text = webVttText.replacingOccurrences(of: ".", with: ".\n")
    return text.replacingOccurrences(of: "&gt;&gt;", with: "\n\n")
}

fileprivate func removeOverSpace(from text: String) -> String {
    return text.replacingOccurrences(of: "  ", with: " ")
}

fileprivate func removeTimes(from text: String) -> String {
    let lines = text.components(separatedBy: .newlines)
    let filtered = lines.filter { !$0.contains("-->")}
    let text = "".appending(filtered.joined(separator: " "))
    return text
}
